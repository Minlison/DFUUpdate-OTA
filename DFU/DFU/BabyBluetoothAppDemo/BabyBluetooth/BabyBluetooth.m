...
- (NSArray <CBPeripheral *>*)retrievePeripheralWithCBServiceUUIDStrings:(NSArray <NSString *>*)UUIDStrings
{
    @try {
        NSMutableArray <CBUUID *>*tmpUUIDS = [NSMutableArray arrayWithCapacity:UUIDStrings.count];
        [UUIDStrings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            CBUUID *uuid = [CBUUID UUIDWithString:obj];
            [tmpUUIDS addObject:uuid];
        }];
        return [self.centralManager retrieveConnectedPeripheralsWithServices:tmpUUIDS];
    } @catch (NSException *exception) {
        BabyLog(@">>> retrievePeripheralWithUUIDString error:%@",exception)
        return nil;
    } @finally {
